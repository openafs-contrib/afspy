...
output from shell commands executed by dao.BosServer
"""
import re
...
def pull_restart_times(ret, output, outerr, parse_param_list, logger):
    """
    parses result from method of same name in dao.BosServer
    """
    ...
    if ret :
        raise BosServerDAOError(outerr, output)
...
def get_filedate(ret, output, outerr, parse_param_list, logger):
    ...
    if "does not exist" in output[0] :
        raise BosServerDAOError(output)
