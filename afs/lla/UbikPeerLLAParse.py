from UbikPeerLLAError import UbikPeerLLAError
import re
...
def get_short_info(rc, output, outerr, parseParamList, Logger) :
    """
    ...
    if "I am sync site" in output[idx] :
        ...
        if "I am not sync site" == output[idx] :
            ...
            raise UbikPeerLLAError("Cannot get Sync/clone info from %s" % output[idx])
        # "I am not sync site"
...
def get_long_info( rc, output, outerr, parseParamList,Logger) :
    """
    ...
    while line_no < len(output)-1  :
        ...
        #raise UbikPeerLLAError("Error Parsing line: %s" % output[line_no])
