...
class TestUbikLLAMethods(unittest.TestCase, BasicTestSetup):
    ...
    def setUp(self):
        ...
        self.SyncSite=self.test_config.get("UbikLLA","SyncSite")
        self.DBPort=self.test_config.get("UbikLLA","DBPort")
        self.DBState=self.test_config.get("UbikLLA","DBState")
        self.allHosts=self.test_config.get("UbikLLA","allDBs").split(",")
        self.allHosts.sort()
        self.not_SyncSite=""
        ...
        for srv in self.allHosts :
            if self.SyncSite != srv :
                ...
                break
        self.min_ubik_dbversion=self.test_config.get("UbikLLA","MinDBVersion")
        self.LLA = UbikPeerLLA.UbikPeerLLA()
        return

    def do_test_getSyncSite(self, servername):
        d=self.LLA.getShortInfo(servername, self.DBPort,_cfg=afs.CONFIG,_user="test")
        SyncSite=d["SyncSite"]
    ...
    def do_test_getAllPeers(self, servername):
        d=self.LLA.getLongInfo(servername, self.DBPort,_cfg=afs.CONFIG,_user="test")
...
if __name__ == '__main__' :
    parse_commandline()
    suite = unittest.TestLoader().loadTestsFromTestCase(TestUbikLLAMethods)
