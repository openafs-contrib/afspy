...
class BsService (BaseService):
    ...
    def getRestartTimes(self,name_or_ip,cached=False):
        """
    ...
    def setRestartTimes(self,this_BosServer,time, restarttype):
        """
        Ask Bosserver about the restart times of the fileserver
        """
        self._bnodeDAO.setRestartTimes(this_BosServer.ipaddrs[0],time, restarttype,  self._CFG.CELL_NAME, self._CFG.Token)
        if self._CFG.DB_CACHE : 
            self.DBManager.setIntoCache(BosServer,this_BosServer,ipaddr=ipaddrs[0])
        return
